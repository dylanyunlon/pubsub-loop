...
def render_room_circular_video(layout: FloorPlan, room_id: str, output_path: str, 
    ...
    render_angles = np.linspace(0, 360, 360 // angle_step)
    ...
    print(f"Total frames in video: {len(frames_repeated)}")
...
def batch_render_composed_layouts(compose_dir, resolution=1080, angle_step=3, fps=30):
    """
    Batch render all composed layouts in a compose directory.
    
    Args:
        compose_dir: Path to the compose directory (e.g., {layout_id}/{type_aug_name}_composed_{pose_aug_name})
        resolution: Video width resolution
        angle_step: Degrees between frames
        fps: Video FPS
    """
    # Validate compose directory
    if not os.path.exists(compose_dir):
        raise ValueError(f"Compose directory not found: {compose_dir}")
    
    layouts_dir = os.path.join(compose_dir, "layouts")
    if not os.path.exists(layouts_dir):
        raise ValueError(f"Layouts directory not found: {layouts_dir}")
    
    # Find all layout JSON files
    layout_files = sorted([f for f in os.listdir(layouts_dir) if f.endswith('.json')])
    
    if not layout_files:
        raise ValueError(f"No layout JSON files found in: {layouts_dir}")
    
    print(f"\n{'='*80}")
    print(f"Batch Rendering Composed Layouts")
    print(f"{'='*80}")
    print(f"Compose directory: {compose_dir}")
    print(f"Found {len(layout_files)} layouts to render")
    print(f"{'='*80}\n")
    ...
    # Process each layout
    for idx, layout_filename in enumerate(layout_files):
        layout_path = os.path.join(layouts_dir, layout_filename)
        compose_id = os.path.splitext(layout_filename)[0]
        
        print(f"\n{'='*80}")
        print(f"Rendering {idx + 1}/{len(layout_files)}: {compose_id}")
        print(f"{'='*80}")
        
        try:
            # Load the composed layout
            with open(layout_path, 'r') as f:
                layout_data = json.load(f)
            layout = dict_to_floor_plan(layout_data)
            
            # Get the first room (assumed to be the main room)
            if not layout.rooms:
                raise ValueError("No rooms found in layout")
            room_id = layout.rooms[0].id
            
            # Create output directory for this composition's video
            videos_dir = os.path.join(compose_dir, "videos")
            os.makedirs(videos_dir, exist_ok=True)
            
            output_path = os.path.join(videos_dir, f"{compose_id}.mp4")
            
            # Render video
            print(f"Rendering circular video for room {room_id}...")
            ...
            print(f"✗ Failed to render {compose_id}: {e}")
            import traceback
            traceback.print_exc()
    ...
    print(f"{'='*80}")
    print(f"Total layouts: {len(layout_files)}")
    print(f"Successful: {successful_renders}")
    print(f"Failed: {len(failed_renders)}")
    
    if failed_renders:
        print(f"\nFailed renders:")
        for failure in failed_renders:
            print(f"  - {failure['compose_id']}: {failure['error']}")
    
    print(f"{'='*80}\n")
    
    return successful_renders, failed_renders


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Batch render all composed layouts from scene_level_type_aug_compose.py")
    ...
    parser.add_argument("--type_aug_name", type=str, required=True,
                       help="Name of the type augmentation")
    ...
    parser.add_argument("--resolution", type=int, default=1080, 
                       help="Video width resolution (default: 1080)")
    parser.add_argument("--angle-step", type=int, default=3, 
                       help="Degrees between frames (default: 3)")
    ...
    try:
        # Construct compose directory path from arguments
        layout_dir = os.path.join(RESULTS_DIR, args.layout_id)
        compose_dir = os.path.join(layout_dir, f"{args.type_aug_name}_composed_{args.pose_aug_name}")
        ...
        print(f"Type Aug Name: {args.type_aug_name}")
        print(f"Pose Aug Name: {args.pose_aug_name}")
        print(f"Compose Directory: {compose_dir}")
        print(f"{'='*80}\n")
        ...
        print(f"Found {len(layout_files)} composed layouts to render\n")
        ...
        for idx, layout_filename in enumerate(layout_files):
            ...
            try:
                ...
                if room is None:
                    raise ValueError(f"Room {args.room_id} not found in layout")
