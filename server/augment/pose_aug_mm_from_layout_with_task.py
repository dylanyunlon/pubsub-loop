...
import copy
from utils import dict_to_floor_plan, export_layout_to_json
import argparse

def generate_samples_using_correction_method(
    scene_save_dir,
    layout_name,
    room_id,
    pick_object_id,
    ...
    # Pre-calculate paths
    output_dir = os.path.join(scene_save_dir, save_dir_name)
    ...
    for i in range(num_samples):
        ...
        for retry in range(max_retries):
            # Clone layout for this attempt
            current_layout = dict_to_floor_plan(copy.deepcopy(base_layout_dict))
            target_room = next(r for r in current_layout.rooms if r.id == room_id)
            
            # Temporary file for this sample attempt (needed for occupancy grid)
            sample_name = f"{save_dir_name}/{i:02d}"
            temp_path = os.path.join(scene_save_dir, f"{sample_name}.json")
            # Ensure dir exists
            os.makedirs(os.path.dirname(temp_path), exist_ok=True)
            export_layout_to_json(current_layout, temp_path)
            
            # 1. Resample Pick Table
            ...
            if pick_table_obj:
                new_objects, success = object_augmentation_pose_object_tree_sim_correction(
                    ...
                    target_room,
                    pick_table_obj,
                    [pick_object_id], # Make sure pick object is reachable
                    layout_file_name=sample_name
                )
                if success and new_objects:
                    target_room.objects = new_objects
                    # Update layout file
                    export_layout_to_json(current_layout, temp_path)
                else:
                    print(f"  Retry {retry}: Failed to resample pick table")
                    continue

            # update the current layout
            current_layout = dict_to_floor_plan(json.load(open(temp_path, "r")))
            target_room = next(r for r in current_layout.rooms if r.id == room_id)
            
            # 2. Resample Place Table (if different from pick table)
            if place_table_id and place_table_id != pick_table_id:
                place_table_obj = next((obj for obj in target_room.objects if obj.id == place_table_id), None)
                
                if place_table_obj: 
                    if place_table_obj.place_id in ["floor", "wall"]:
                        # place_table_obj is the table itself
                        new_objects, success = object_augmentation_pose_object_tree_sim_correction(
                            current_layout,
                            target_room,
                            place_table_obj,
                            [], # No specific object needs to be reachable on the place table initially
                            layout_file_name=sample_name
                        )
                    else:
                        # place_table_obj is on another object (the table)
                        place_table_id = place_table_obj.place_id
                        place_table_parent_obj = next((obj for obj in target_room.objects if obj.id == place_table_id), None)
                        new_objects, success = object_augmentation_pose_object_tree_sim_correction(
                            current_layout,
                            target_room,
                            place_table_parent_obj,
                            [place_table_obj.id],
                            layout_file_name=sample_name
                        )
                    ...
                    if success and new_objects:
                        ...
                        print(f"  Retry {retry}: Failed to resample place table")
                        continue

            # If we got here, sampling was successful
            success_sample = True
            print(f"  Sample {i} generated successfully")
            break
        
        if not success_sample:
            print(f"Warning: Could not generate valid sample {i} after {max_retries} retries")
...
if __name__ == "__main__":
    ...
    generate_samples_using_correction_method(
