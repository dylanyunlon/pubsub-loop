...
def main():
    ...
    num_samples_per_pose = 3

    for layout_dict_current in meta_layouts:

        all_spawn_pos = []
        all_spawn_angles = []
        layout_name = layout_dict_current["layout_id"]

        for i in range(num_samples_per_pose):
            spawn_pos, spawn_angles = sample_robot_spawn(
                scene_save_dir, layout_name, room_id, 1, debug_dir
            )
        ...
        layout_dict_current["spawn_pose"] = spawn_pose_dict

    with open(os.path.join(log_dir, f"spawn_pose.json"), "w") as f:
        json.dump(meta_layouts, f, indent=4)
