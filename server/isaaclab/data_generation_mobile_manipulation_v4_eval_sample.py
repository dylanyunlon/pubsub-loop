...
def main():
    ...
    for i in range(num_samples):
        spawn_pos, spawn_angles = sample_robot_spawn(
            scene_save_dir, layout_id, room_id, 1, debug_dir
        )
        all_spawn_pos.append(spawn_pos)
        all_spawn_angles.append(spawn_angles)

    all_spawn_pos = torch.cat(all_spawn_pos, dim=0)
    all_spawn_angles = torch.cat(all_spawn_angles, dim=0)

    print(f"all_spawn_pos: {all_spawn_pos.shape}")
    print(f"all_spawn_angles: {all_spawn_angles.shape}")

    spawn_pose_save_json_path = os.path.join(log_dir, "spawn_pose.json")

    all_spawn_pos = all_spawn_pos.cpu().numpy().tolist()
    all_spawn_angles = all_spawn_angles.cpu().numpy().tolist()

    spawn_pose_save_json = {
        "spawn_pos": all_spawn_pos,
        "spawn_angles": all_spawn_angles,
    }


    with open(spawn_pose_save_json_path, "w") as f:
        json.dump(spawn_pose_save_json, f, indent=4)

    print(f"spawn_pose_save_json_path: {spawn_pose_save_json_path}")
    assert False
            ...
            if iteration % T_grasp == T_grasp - 1:
                ...
                for env_i in range(num_envs):
                    ...
                    if is_place_success and collected_data_episodes < args_cli.num_demos:
                        for data_i, iteration_i in enumerate(range(T_init+1, min(place_success_T_envs[env_i] + T_drop, T_grasp))):
