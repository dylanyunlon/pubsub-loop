    ...
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        while not collector_interface.is_stopped():
            ...
            if iteration % T_grasp == 0:
                ...
                grasp_ee_goal = ee_goals_copy.clone()
                grasp_ee_goal[2] = grasp_ee_goal[2] - grasp_down_offset
            ...
            if not is_reach_grasp_goal:
                is_reach_grasp_goal = is_ee_reach_goal(grasp_ee_goal, ee_frame_data)
                if is_reach_grasp_goal:
                    close_iters = T_close_gap
            
            if not is_reach_grasp_goal:
                ee_goal = grasp_ee_goal
                gripper_switch = 1
            else:
                gripper_switch = -1
                if close_iters > 0:
