...
parser.add_argument("--task", type=str, default="Isaac-Lift-Single-Obj-Scene-Franka-IK-Abs-v0-vis", help="Name of the task.")
...
parser.add_argument("--aug_name", type=str, required=True, help="Name of the augmentation.")
...
def main():
    ...
    log_dir = os.path.abspath(os.path.join("./robomimic_data/", args_cli.task+args_cli.aug_name))
    ...
    target_object_name = args_cli.target_object_name
    place_object_name = args_cli.place_object_name
    table_object_name = args_cli.table_object_name
    num_envs = args_cli.num_envs

    aug_name = args_cli.aug_name
    ...
    camera_height_vis = 1.2
    camera_to_base_vis = 1.2
    camera_forward_vis = 0.0
            ...
            if iteration % T_grasp > T_init and current_data_idx < 5:
                ...
                for env_i in range(min(num_envs, 8)):
                    camera_frame_envs.append(
                        np.concatenate([
                            iteration_status["obs/rgb_left_vis"][env_i].cpu().numpy(),
                            iteration_status["obs/rgb_right_vis"][env_i].cpu().numpy(),
                        ], axis=1)
                    )
