...
parser.add_argument("--task", type=str, default="Isaac-Mobile-Manipulation-Obj-Scene-Franka-IK-Abs-v3-vis", help="Name of the task.")
parser.add_argument("--num_demos", type=int, default=8, help="Number of episodes to store in the dataset.")
...
def main():
    ...
    # layout_id = "layout_59a4e6a3"
    # pick_table_id = "room_1848f09b_coffeetable_b2f9227b"
    # pick_object_id = "room_1848f09b_cup_4_a67a949b"
    # place_table_id = "room_1848f09b_sideboard_d64a9e86"
    # room_id = "room_1848f09b"

    # layout_id = "layout_4bdf835f"
    # pick_table_id = "room_1848f09b_coffeetable_b2f9227b"
    # pick_object_id = "room_1848f09b_cup_3_4b362e5a"
    # place_table_id = "room_1848f09b_sideboard_d64a9e86"
    # room_id = "room_1848f09b"

    # layout_id = "layout_caae5ce5"
    # pick_table_id = "room_1848f09b_coffeetable_b2f9227b"
    # pick_object_id = "room_1848f09b_mug_c369d8db"
    # place_table_id = "room_1848f09b_sideboard_d64a9e86"
    # room_id = "room_1848f09b"

    layout_id = "layout_3dea5ab1"
    pick_table_id = "room_2adee539_nightstand_eea22366"
    pick_object_id = "room_2adee539_mug_25cacda3"
    place_table_id = "room_2adee539_desk_68063726"
    ...
    with contextlib.suppress(KeyboardInterrupt):
        while not collector_interface.is_stopped():
            ...
            if iteration % T_grasp == 0:
                ...
                # )
                spawn_pos_list = [1.22, 2.35]
                spawn_angles_list = [0.]
                spawn_pos = torch.tensor(spawn_pos_list, device=env.sim.device).reshape(1, 2).repeat(num_envs, 1).float()
                spawn_angles = torch.tensor(spawn_angles_list, device=env.sim.device).reshape(1, 1).repeat(num_envs, 1).float()
            ...
            for env_i in range(num_envs):
                ...
                if iteration % T_grasp < T_init:
                    ...
                    is_reach_grasp_goal_down_envs[env_i] = is_reach(pick_ee_goals_down[env_i, :], ee_frame_data_pose[env_i], 
                    ...
                    if is_reach_grasp_goal_down_envs[env_i]:
                        reach_grasp_up_traj, success = curobo_plan_traj(
                            ...
                            max_interpolation_step_distance=0.0002,
                            max_length=250,
                            interpolate=True,
                            current_ee_quat=ee_frame_data_quat[env_i:env_i+1],
                        )
                ...
                if iteration % T_grasp == 0:
                    ...
                    grasp_up_goals_iter = reach_grasp_up_traj_envs[env_i][min(max(iteration - reach_grasp_up_T_envs[env_i] - T_hold, 0), len(reach_grasp_up_traj_envs[env_i]) - 1)].reshape(1, 7)

                    actions = torch.cat([
                        get_action_relative(fixed_support_frame_data_pos[env_i:env_i+1], fixed_support_frame_data_quat[env_i:env_i+1], 
                        robot_base_pick_pos[env_i:env_i+1], robot_base_pick_quat[env_i:env_i+1]),
                        # get_action_relative(ee_frame_data_pos[env_i:env_i+1], ee_frame_data_quat[env_i:env_i+1], 
            ...
            if current_data_idx < 5 and iteration % T_grasp >= T_init:
                ...
                for env_i in range(min(num_envs, 8)):
                    camera_frame_envs.append(
                        np.concatenate([
                            obs_dict["policy"]["rgb_front_left_vis"][env_i].cpu().numpy(),
                            obs_dict["policy"]["rgb_front_right_vis"][env_i].cpu().numpy(),
                            obs_dict["policy"]["rgb_back_left_vis"][env_i].cpu().numpy(),
                            obs_dict["policy"]["rgb_back_right_vis"][env_i].cpu().numpy(),
                            obs_dict["policy"]["rgb_top_view"][env_i].cpu().numpy(),
                        ], axis=1)
                    )
            ...
            if iteration % T_grasp == T_grasp - 1:
                ...
                if current_data_idx < 5:
                    ...
                    video_path_wall = os.path.join(cameras_debug_dir, f"{current_data_idx:0>3d}_cameras_wall.mp4")
                    imageio.mimwrite(video_path_wall, camera_list_wall, fps=30)
                    print(f"saving {current_data_idx} data to video: {video_path_wall}")
                ...
                if args_cli.visualize_figs:
                    try:
                        ...
                        if hasattr(main, 'pick_trajectory_viz_data') and len(main.pick_trajectory_viz_data) > 0:
                            ...
                            merged_pick_viz_filename = f"merged_pick_trajectories_{layout_id}_{room_id}_{current_data_idx:03d}_{timestamp}.png"
                        ...
                        if hasattr(main, 'place_trajectory_viz_data') and len(main.place_trajectory_viz_data) > 0:
                            ...
                            import time
                            timestamp = time.strftime("%Y%m%d_%H%M%S")
                            merged_place_viz_filename = f"merged_place_trajectories_{layout_id}_{room_id}_{current_data_idx:03d}_{timestamp}.png"
                        ...
                        if (hasattr(main, 'pick_trajectory_viz_data') and len(main.pick_trajectory_viz_data) > 0 and 
                            ...
                            import time
                            timestamp = time.strftime("%Y%m%d_%H%M%S")
                            combined_viz_filename = f"combined_pick_place_trajectories_{layout_id}_{room_id}_{current_data_idx:03d}_{timestamp}.png"
