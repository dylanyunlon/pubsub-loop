...
def get_grasp_transforms_with_additional_transform(scene_save_dir, target_object_name, base_pos, additional_transform_dict):
    ...
    for object_name in mesh_dict_list.keys():
        ...
        mesh_dict_list[object_name]["mesh"].apply_transform(object_transform)
...
def main():
    ...
    log_dir = os.path.abspath(os.path.join("./logs/robomimic", args_cli.task+"-100-v3"))
    ...
    # # banana
    # scene_save_dir = os.path.join(SERVER_ROOT_DIR, "results/layout_f5a18db0")
    # target_object_name = "room_744fcab1_small_banana_a836b09f"
    # base_pos = [7.59, 6.76, 0.9]

    # box
    scene_save_dir = os.path.join(SERVER_ROOT_DIR, "results/layout_c102a6fc")
    target_object_name = "room_744fcab1_cheez_it_crackers_box_57039860"
    base_pos = [7.59, 7.60, 0.75]
    ...
    with contextlib.suppress(KeyboardInterrupt):
        while not collector_interface.is_stopped():
            ...
            if iteration % T_grasp == 0:
                ...
                random_pos = np.random.uniform(-0.1, 0.1, 2)
            ...
            if iteration % T_grasp == T_init + 1:
                ...
                while True:
                    grasp_transforms = get_grasp_transforms_with_additional_transform(scene_save_dir, target_object_name, robot_base_w[0, :3].tolist(), additional_transform_dict)
                    ee_goals_translate_w = grasp_transforms[:, :3, 3].reshape(-1, 3)
                ...
                while True:
                    ...
                    place_location = place_location_w_up.clone()
                    place_location = place_location - robot_base_w[0, :3]
            ...
            if iteration % T_grasp > T_init:
                ...
                if not is_reach_grasp_up_goal:
                    ee_goal = curobo_target_ee_pos[min(max(iteration - T_init, 0), len(curobo_target_ee_pos) - 1)] if curobo_target_ee_pos is not None else grasp_ee_goal_up
                    gripper_switch = 1
                elif not is_reach_grasp_down_goal:
