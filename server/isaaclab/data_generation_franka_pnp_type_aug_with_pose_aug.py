...
from constants import SERVER_ROOT_DIR, ROBOMIMIC_ROOT_DIR, M2T2_ROOT_DIR, RESULTS_DIR
...
def main():
    ...
    original_table_object_name = parent_object_ids[0]
    room_id = room.id

    updated_task_decomposition = policy_analysis["updated_task_decomposition"]
    for step_dict in updated_task_decomposition:
        if step_dict["action"] == "pick":
            original_target_object_name = step_dict["target_object_id"]
        elif step_dict["action"] == "place":
            original_target_object_name = step_dict["target_object_id"]
            original_place_object_name = step_dict["location_object_id"]

    print(f"Pick object ID: {original_target_object_name}")
    print(f"Place object ID: {original_place_object_name}")
    print(f"Table object ID: {original_table_object_name}")
    ...
    print(f"aug_pose_json_path: {aug_pose_json_path}")
