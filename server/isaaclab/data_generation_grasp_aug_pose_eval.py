...
def main():
    ...
    log_dir = os.path.abspath(os.path.join("./logs/robomimic", args_cli.task+"-aug-pose-v2-reach-20x2-train-set"))
    # aug 100
    robomimic_policy_path = os.path.join(ROBOMIMIC_ROOT_DIR, "diffusion_policy_trained_models/dp_mug_aug_pose_v2_reach_80x2/20250820134212/models/model_epoch_1400.pth")
    ...
    train_layout_aug_ids = layout_aug_ids[:int(len(layout_aug_ids) * train_set_ratio)]
    ...
    for layout_name in object_transform_layouts_dict_eval.keys():
        ...
        while True:
            ...
            is_place_stay = torch.norm(place_pos_xy_w - place_xy_w) < 0.04
            is_target_reach = torch.norm(target_pos_xy_w - place_xy_w) < 0.04
    ...
    print(f"Success rate: {success_count / total_layout_count}")
