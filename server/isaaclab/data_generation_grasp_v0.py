...
def main():
    ...
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        while not collector_interface.is_stopped():
            ...
            elif (iteration % T_grasp > T_start_grasp or gripper_stable) and iteration % T_grasp < T_grasp - 500 - grasp_offset:
                # reset actions
                original_ee_goals = ee_goals_copy.clone()
                grasp_end_actual = (T_grasp - 500 - grasp_offset)
                grasp_start_actual = (T_start_grasp - grasp_offset)
            ...
            if iteration % T_grasp == T_grasp - 1 - grasp_offset:
            ...
            stay_close_length = 50
            if len(gripper_gap_history) > stay_close_length:
                gripper_gap_history.pop(0)
            
            # Check if gripper is stable (last 5 values have small variance)
            if not gripper_stable and iteration % T_grasp > T_start_close and len(gripper_gap_history) == stay_close_length:
                gap_min = min(gripper_gap_history)
                gap_max = max(gripper_gap_history)
                if gap_max - gap_min <= 0.00001:
                    gripper_stable = True
                    grasp_offset = T_start_grasp - (iteration % T_grasp)

            com_state = rigid_object_collection.data.object_com_state_w

            success = False
            if iteration % T_grasp > T_start_down:
            ...
            if iteration == -1:
                print(f"pose {current_goal_idx}: success = {success}")
