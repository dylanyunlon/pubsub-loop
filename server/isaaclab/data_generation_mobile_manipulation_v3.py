...
def check_unified_robot_collision(pos, quat, scene_occupancy_fn, room_bounds):
    """
    ...
    robot_occupancy_fn = occupancy_map(forward, side, yaw, offset=CollisionCheckingConfig.ROBOT_OCCUPANCY_OFFSET)
...
def sample_robot_spawn(
    scene_save_dir, layout_id, room_id, num_envs
):
    """
...
def visualize_trajectory_planning(
    ...
    """
    Visualize trajectory planning results including occupancy grid, RRT tree, and final trajectory.
    ...
    # 7. Set equal aspect ratio and grid
    ax.set_aspect('equal')
    ax.grid(True, alpha=0.3)
...
def plan_robot_traj(start_pos, start_quat, end_pos, end_quat,
    scene_save_dir, layout_name, room_id,
    max_move_distance_per_step = 0.01,
    ...
    goal_threshold_pos = 0.02  # Slightly more tolerant
    goal_threshold_rot = np.deg2rad(5.0)  # More tolerant
    step_size_pos = max_move_distance_per_step * 3  # Larger steps
    step_size_rot = np.deg2rad(max_rotate_degree_per_step * 2)  # Larger rotation steps
    ...
    if check_collision_unified(start_pos_np, start_quat_np):
        ...
        return torch.tensor([[*start_pos_np, *start_quat_np]], dtype=torch.float, device="cuda")
    ...
    # Create visualization (simplified for speed)
    try:
        debug_dir = os.path.join(os.path.dirname(scene_save_dir), "debug", "trajectory_planning")
        os.makedirs(debug_dir, exist_ok=True)
...
def get_grasp_transforms(layout_json_path, target_object_name, base_pos):
    ...
    while True:
        ...
        print(f"dist < {max_dist_to_mesh}: {np.sum(dists < max_dist_to_mesh)}")
        grasp_transforms = grasp_transforms[dists < max_dist_to_mesh, :]
    ...
    with contextlib.suppress(KeyboardInterrupt):
        while True:
            ...
            if iteration % T_grasp == 0:
                ...
                joint_pos[:, joint_idx_base_angle] = spawn_angles
            ...
            for env_i in range(num_envs):
                ...
                if iteration % T_grasp == T_init:
                    ...
                    reach_pick_base_traj_envs[env_i] = plan_robot_traj(
                        fixed_support_frame_data_pos[env_i, :3], fixed_support_frame_data_quat[env_i],
                        robot_base_pick_pose[env_i, :3], robot_base_pick_pose[env_i, 3:7],
                        scene_save_dir, layout_id, room_id
                    )
                    reach_pick_base_T_envs[env_i] = iteration
                    robot_base_pick_pose[env_i, :] = reach_pick_base_traj_envs[env_i][-1, :]
                ...
                if not is_reach_pick_goal_envs[env_i]:
                    ...
                    is_reach_pick_goal_envs[env_i] = is_reach(robot_base_pick_pose[env_i], fixed_support_frame_data_pose[env_i], ignore_z=True, loc_threshold=0.05, rot_threshold=5.0)
                    ...
                    is_reach_grasp_goal_down_envs[env_i] = is_reach(pick_ee_goals_down[env_i, :], ee_frame_data_pose[env_i], loc_threshold=0.05, rot_threshold=30.0)
                    ...
                    if is_reach_grasp_goal_down_envs[env_i]:
                        reach_grasp_up_traj, success = curobo_plan_traj(
                            ...
                            interpolate=True,
                            max_interpolation_step_distance=0.002
                        )
                    ...
                    if is_reach_grasp_goal_up_envs[env_i]:
                        ...
                        #     max_interpolation_step_distance=0.01,
                        # )
                        reach_place_base_traj_envs[env_i] = plan_robot_traj(
                            fixed_support_frame_data_pos[env_i, :3], fixed_support_frame_data_quat[env_i],
                        ...
                        robot_base_place_pose[env_i, :] = reach_place_base_traj_envs[env_i][-1, :]
                    ...
                    is_reach_place_goal_envs[env_i] = is_reach(robot_base_place_pose[env_i], fixed_support_frame_data_pose[env_i], ignore_z=True, loc_threshold=0.05, rot_threshold=5.0)
                ...
                if iteration % T_grasp == 0:
                    ...
                    actions = torch.cat([
                        get_action_relative(fixed_support_frame_data_pos[env_i:env_i+1], fixed_support_frame_data_quat[env_i:env_i+1], 
                            fixed_support_frame_data_pos_initial.reshape(1, 3), 
                            fixed_support_frame_data_quat_initial.reshape(1, 4)),
                        get_action_relative(ee_frame_data_pos[env_i:env_i+1], ee_frame_data_quat[env_i:env_i+1], 
                        ee_frame_data_pos_target_pos[env_i:env_i+1], ee_frame_data_pos_target_quat[env_i:env_i+1]),
                        torch.tensor([1.0,], device=env.sim.device).reshape(1, 1),
                    ], dim=-1)
