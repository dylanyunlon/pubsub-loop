...
parser.add_argument("--spawn_pose_post_fix", type=str, default="", help="Spawn pose post fix")
parser.add_argument("--policy_paths", type=str, nargs='+', default=[], help="Path(s) to the policy checkpoint(s)")
...
def main():
    ...
    spawn_pose_post_fix = args_cli.spawn_pose_post_fix

    eval_spawn_pose_json_path = os.path.join("./robomimic_data", spawn_pose_post_fix, "spawn_pose.json")
    ...
    robomimic_policy_paths = args_cli.policy_paths
    rollout_policies = []
    for robomimic_policy_path in robomimic_policy_paths:
        rollout_policy, ckpt_dict = FileUtils.policy_from_checkpoint(device="cpu", ckpt_path=robomimic_policy_path)
        rollout_policies.append(rollout_policy)
    ...
    with contextlib.suppress(KeyboardInterrupt):
        while True:
            ...
            if iteration % T_grasp == 0:
                ...
                current_stage = 0
                rollout_policy = rollout_policies[current_stage]
                rollout_policy.policy.to_device("cuda")
                rollout_policy.start_episode()

                success_envs = [False] * num_envs

                # Initialize stage switch counter
                num_switch_steps = 16  # Number of consecutive steps needed to switch stage
                min_stage_iterations = 100  # Minimum iterations each stage must run before allowing switch
                stage_switch_counter = 0
            ...
            for env_i in range(num_envs):
                ...
                if iteration % T_grasp < T_init:
                    ...
                    current_stage_iterations += 1
                    
                    # Check if stage should be finished (requires consecutive detections AND minimum iterations)
                    stage_signal = actions[0, -1] > 0.5
                    if stage_signal:
                        stage_switch_counter += 1
                        # if stage_switch_counter % 5 == 0 or stage_switch_counter >= num_switch_steps:
                        #     print(f"Stage switch signal detected: {stage_switch_counter}/{num_switch_steps} consecutive steps")
                    else:
                        # if stage_switch_counter > 0:
                        #     print(f"Stage switch signal lost (was at {stage_switch_counter}/{num_switch_steps})")
                        stage_switch_counter = 0  # Reset counter if signal is not detected
                    
                    # Only mark as finished if we have enough consecutive detections AND minimum iterations
                    stage_finished = (stage_switch_counter >= num_switch_steps and 
                                    current_stage_iterations >= min_stage_iterations)
                    
                    # Debug: Print when consecutive detections are met but waiting for minimum iterations
                    if stage_switch_counter >= num_switch_steps and current_stage_iterations < min_stage_iterations:
                        if current_stage_iterations % 20 == 0:  # Print every 20 iterations to avoid spam
                            print(f"Stage switch conditions: detections met ({stage_switch_counter}/{num_switch_steps}), "
                                  f"waiting for min iterations ({current_stage_iterations}/{min_stage_iterations})")
            ...
            if stage_finished and current_stage < len(rollout_policies) - 1:
                rollout_policy.policy.to_device("cpu")
                current_stage += 1
                print(f"switch policy for the next stage {current_stage} (after {stage_switch_counter} consecutive detections, {current_stage_iterations} iterations)")
                rollout_policy = rollout_policies[current_stage]
                rollout_policy.policy.to_device("cuda")
                rollout_policy.start_episode()
                # Reset counters for next stage transition
                stage_switch_counter = 0
                current_stage_iterations = 0
