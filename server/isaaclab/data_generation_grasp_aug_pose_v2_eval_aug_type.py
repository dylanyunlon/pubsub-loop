...
parser.add_argument("--type_candidate_id", type=str, default=None, help="Specific type candidate ID to evaluate (if not provided, evaluates all).")
...
def sample_robot_location(
    scene_save_dir, layout_name, room_id, layout_json_path,
...
def main():
    ...
    # Parameters for iterating through type and pose augmentations
    type_aug_name = args_cli.type_aug_name
    pose_aug_name = args_cli.pose_aug_name
    specific_type_candidate_id = args_cli.type_candidate_id
    
    log_dir = os.path.abspath(os.path.join("./logs_eval", args_cli.task+f"_{type_aug_name}_{pose_aug_name}_{os.path.abspath(args_cli.ckpt_path).replace('/', '_')}"))
    # aug 100
    robomimic_policy_path = args_cli.ckpt_path
    ...
    # Store original object names from CLI arguments
    ...
    # Load type candidates metadata to get all type candidate IDs
    ...
    print(f"Found {type_metadata['total_candidates']} type candidates to evaluate")
    
    # Filter type candidates based on command line argument
    if specific_type_candidate_id:
        # Process only the specified type candidate
        candidate_infos = [info for info in type_metadata["candidates"] if info["layout_id"] == specific_type_candidate_id]
        if not candidate_infos:
            raise ValueError(f"Type candidate {specific_type_candidate_id} not found in metadata")
        print(f"Processing specific type candidate: {specific_type_candidate_id}")
    else:
        assert False, "Not implemented"
        # Process all type candidates (but this should be used carefully due to simulation context limitations)
        candidate_infos = type_metadata["candidates"]
        print(f"Processing all {len(candidate_infos)} type candidates")
    
    # Process each type candidate
    all_success_dict = {}
    all_type_candidate_stats = {}
    ...
    for candidate_info in candidate_infos:
        ...
        try:
            # Path to the pose augmentation results for this type candidate
            type_candidate_base_dir = os.path.join(scene_save_dir, type_aug_name, type_candidate_id)
            pose_aug_dir = os.path.join(type_candidate_base_dir, pose_aug_name)
            aug_pose_json_path = os.path.join(type_candidate_base_dir, f"all_augmented_layouts_info_{pose_aug_name}.json")
            
            if not os.path.exists(aug_pose_json_path):
                print(f"Warning: No pose augmentation results found for type candidate {type_candidate_id} at {aug_pose_json_path}")
                continue
            
            with open(aug_pose_json_path, "r") as f:
                candidate_augmented_layouts_info = json.load(f)
            
            # Get candidate data
            mass_dict = candidate_augmented_layouts_info["mass_dict"]
            usd_collection_dir = candidate_augmented_layouts_info["usd_collection_dir"]
            # usd_collection_dir: cut the substring from "results/" to the end
            if "results/" in usd_collection_dir:
                usd_collection_dir = usd_collection_dir[usd_collection_dir.find("results/"):]
            object_transform_layouts_dict = candidate_augmented_layouts_info["object_transform_dict"]
            
            # Map layout IDs to their JSON file paths for this type candidate
            layout_json_paths = {}
            for layout_aug_id in object_transform_layouts_dict.keys():
                layout_json_path = os.path.join(pose_aug_dir, f"{layout_aug_id}.json")
                if os.path.exists(layout_json_path):
                    layout_json_paths[layout_aug_id] = layout_json_path
                else:
                    print(f"Warning: Layout JSON file not found: {layout_json_path}")
            
            if len(layout_json_paths) == 0:
                print(f"No valid layouts found for type candidate {type_candidate_id}, skipping...")
                continue
            ...
            print(f"Object ID mapping for type candidate {type_candidate_id}:")
            ...
            # Get all object rigids from the first layout
            all_object_rigids = list(object_transform_layouts_dict[
                next(iter(object_transform_layouts_dict.keys()))
            ].keys())
            
            print(f"Processing type candidate {type_candidate_id} with {len(object_transform_layouts_dict)} layouts")
            
            # Create environment configuration for this type candidate
            ...
            env_init_config_yaml = os.path.join(log_dir, "params", f"env_init_{type_candidate_id}.yaml")
            os.makedirs(os.path.dirname(env_init_config_yaml), exist_ok=True)
            dump_yaml(env_init_config_yaml, config_dict)

            # Create environment for this type candidate
            ...
            # Prepare layout list for this type candidate with repetitions
            sorted_layout_names = sorted(list(object_transform_layouts_dict.keys()))
            attempts_per_layout_dict = {layout_name: 0 for layout_name in sorted_layout_names}
            repeated_layout_names = [layout_name for layout_name in sorted_layout_names for _ in range(attempts_per_layout)]
            
            print(f"Evaluating {len(repeated_layout_names)} layout attempts for type candidate {type_candidate_id}")

            # Simulate environment for all layouts in this type candidate
            for layout_name in repeated_layout_names:
                ...
                while True:
                    ...
                    if iteration == 0:
                        ...
                        object_transform_dict = object_transform_layouts_dict[layout_name]

                        # compute place location and related variables
                        layout_json_path = layout_json_paths[layout_name]
                        print(f"layout_json_path: {layout_json_path}")
                    ...
                    if iteration % T_grasp == 0:
                        # Create unique video name with type candidate prefix
                        video_name = f"{type_candidate_id}_{layout_name}_{attempts_per_layout_dict[layout_name]}.mp4"
                        imageio.mimwrite(os.path.join(eval_dir, video_name), camera_list, fps=30)
                        break
            
            # Store results for this type candidate
            all_success_dict[type_candidate_id] = success_dict.copy()
            all_type_candidate_stats[type_candidate_id] = {
                "layouts": list(layout_json_paths.keys()),
                "total_layouts": len(layout_json_paths),
                "success_dict": success_dict.copy()
            }
            
            # Close environment for this type candidate
            env.close()
            
            print(f"✓ Completed evaluation for type candidate {type_candidate_id}")
            print(f"  Layouts evaluated: {len(layout_json_paths)}")
            print(f"  Successes: {sum(success_dict.values())}")
            
        except Exception as e:
            print(f"✗ Failed to evaluate type candidate {type_candidate_id}: {str(e)}")
            continue
    ...
    if specific_type_candidate_id:
        # Single type candidate results
        if specific_type_candidate_id in all_success_dict:
            success_dict = all_success_dict[specific_type_candidate_id]
            stats = all_type_candidate_stats[specific_type_candidate_id]
            
            print(f"Type candidate: {specific_type_candidate_id}")
            print(f"Total layouts: {stats['total_layouts']}")
            print(f"Total successes: {sum(success_dict.values())}")
            print(f"Success rate: {sum(success_dict.values()) / stats['total_layouts']:.3f}")
            
            print(f"\nPer-layout results:")
            for layout_name in sorted(success_dict.keys()):
                successes = success_dict[layout_name]
                print(f"  {layout_name}: {successes}/{attempts_per_layout}")
                
            # Save results to file for the wrapper script to read
            results_data = {
                "type_candidate_id": specific_type_candidate_id,
                "total_layouts": stats['total_layouts'],
                "total_successes": sum(success_dict.values()),
                "success_rate": sum(success_dict.values()) / stats['total_layouts'],
                "attempts_per_layout": attempts_per_layout,
                "layout_results": {
                    layout_name: {
                        "successes": successes,
                        "attempts": attempts_per_layout
                    }
                    for layout_name, successes in success_dict.items()
                },
                "evaluation_completed": True
            }
            
            # Save to results file
            results_file = os.path.join(log_dir, f"results_{specific_type_candidate_id}.json")
            with open(results_file, "w") as f:
                json.dump(results_data, f, indent=4)
            print(f"\nResults saved to: {results_file}")
            
        else:
            print(f"No results for type candidate {specific_type_candidate_id}")
            # Save empty results
            results_data = {
                "type_candidate_id": specific_type_candidate_id,
                "total_layouts": 0,
        ...
        # Multi type candidate results
        total_layouts = sum(stats['total_layouts'] for stats in all_type_candidate_stats.values())
        total_successes = sum(sum(stats['success_dict'].values()) for stats in all_type_candidate_stats.values())
        
        print(f"Total type candidates processed: {len(all_type_candidate_stats)}")
        print(f"Total layouts: {total_layouts}")
        print(f"Total successes: {total_successes}")
        if total_layouts > 0:
            print(f"Overall success rate: {total_successes / total_layouts:.3f}")
        
        print(f"\nPer-type-candidate results:")
        for type_candidate_id, stats in all_type_candidate_stats.items():
            candidate_successes = sum(stats['success_dict'].values())
            candidate_success_rate = candidate_successes / stats['total_layouts'] if stats['total_layouts'] > 0 else 0.0
            print(f"  {type_candidate_id}: {candidate_success_rate:.3f} ({candidate_successes}/{stats['total_layouts']})")
