                ...
                for env_i in range(min(num_envs, 8)):
                    camera_frame_envs.append(
                        np.concatenate([
                            ...
                            iteration_status["obs/rgb_front_left_vis"][env_i].cpu().numpy(),
                            iteration_status["obs/rgb_front_right_vis"][env_i].cpu().numpy(),
                            iteration_status["obs/rgb_back_left_vis"][env_i].cpu().numpy(),
                            iteration_status["obs/rgb_back_right_vis"][env_i].cpu().numpy(),
                            iteration_status["obs/rgb_top_view"][env_i].cpu().numpy(),
                        ], axis=1)
                    )
