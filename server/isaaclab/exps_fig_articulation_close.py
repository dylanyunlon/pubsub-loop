...
parser.add_argument("--task", type=str, default="Isaac-Lift-Single-Obj-Scene-Franka-IK-Abs-v0-art", help="Name of the task.")
...
def get_grasp_transforms(layout_json_path, target_object_name, base_pos):
    ...
    while True:
        ...
        grasp_transforms = grasp_transforms[:100]
        grasp_x = grasp_transforms[:, 0, 3].reshape(-1)
        grasp_transforms = grasp_transforms[np.argsort(grasp_x)]
...
def process_action(original_ee_goals, gripper_switch):
    ...
    gripper_vel = torch.tensor([gripper_switch], dtype=torch.float, device="cpu").reshape(-1, 1)

    actions = torch.cat([abs_pose_pos, abs_pose_quat, gripper_vel], dim=-1)

    return actions

def is_ee_reach_goal(ee_goal, ee_frame_data, env_i, loc_success_threshold=0.03):
    ...
    loc_success = delta_pos.abs().max() < loc_success_threshold
...
def sample_robot_location(
    ...
    robot_min_dist_to_object = 0.35
    robot_height_offset = 0.2
    grid_res = 0.02
    num_sample_points = 100000
    
    layout_json_path = os.path.join(scene_save_dir, f"{layout_name}_edit_moved.json")
    ...
    for i in range(num_envs):
        ...
        robot_base_pos = torch.tensor([robot_x, robot_y, robot_z], dtype=torch.float, device="cpu")
...
def main():
    ...
    log_dir = os.path.abspath(os.path.join("./robomimic_data/", args_cli.task))
    ...
    room_id = "room_d9517921"
    # target_object_name = "room_d9517921_perfume_72117225"
    # place_object_name = "room_d9517921_bowl_1e707f05"

    target_object_name = "room_d9517921_bowl_1e707f05"
    place_object_name = "room_d9517921_perfume_72117225"
    table_object_name = "room_d9517921_dresser_08d9ad67"
    num_envs = 1

    close_traj_point_1_w = np.array([0.86, 1.89, 0.75]).reshape(1, 3)
    close_traj_point_2_w = np.array([0.86, 1.89, 0.55]).reshape(1, 3)
    close_traj_point_3_w = np.array([0.56, 1.89, 0.55]).reshape(1, 3)


    # close_traj_point_1_w = np.array([0.86, 2.45, 0.75]).reshape(1, 3)
    # close_traj_point_2_w = np.array([0.86, 2.45, 0.55]).reshape(1, 3)
    # close_traj_point_3_w = np.array([0.56, 2.45, 0.55]).reshape(1, 3)

    close_traj_point_1_w = torch.tensor(close_traj_point_1_w, device="cpu").float()
    close_traj_point_2_w = torch.tensor(close_traj_point_2_w, device="cpu").float()
    close_traj_point_3_w = torch.tensor(close_traj_point_3_w, device="cpu").float()

    layout_json_path = os.path.join(scene_save_dir, f"{layout_id}_edit_moved.json")
    ...
    camera_height_vis = 1.0
    camera_to_base_vis = 0.75
    camera_forward_vis = -0.75
    ...
    with contextlib.suppress(KeyboardInterrupt):
        while True:
            ...
            if iteration % T_grasp == 0:
                ...
                place_location_w = np.array([0.57, 2.13, 0.77]).reshape(1, 3)
                place_location_w = torch.tensor(place_location_w, device="cpu").float()
                ...
                # place location to each env coordinate
                close_traj_point_1_pos, close_traj_point_1_quat = math_utils.combine_frame_transforms(
                    envs_pos, envs_quat,
                    close_traj_point_1_w.reshape(1, 3).repeat(num_envs, 1), torch.tensor([1, 0, 0, 0], device=env.sim.device).reshape(1, 4).repeat(num_envs, 1)
                )
                close_traj_point_1_w_env = torch.cat([close_traj_point_1_pos, close_traj_point_1_quat], dim=-1)

                # place location to each robot coordinate
                close_traj_point_1_r_pos, close_traj_point_1_r_quat = math_utils.subtract_frame_transforms(
                    robot_base_w[:, :3], robot_base_w[:, 3:7],
                    close_traj_point_1_w_env[:, :3], close_traj_point_1_w_env[:, 3:7]
                )
                close_traj_point_1_r = torch.cat([close_traj_point_1_r_pos, close_traj_point_1_r_quat], dim=-1)

                # place location to each env coordinate
                close_traj_point_2_pos, close_traj_point_2_quat = math_utils.combine_frame_transforms(
                    envs_pos, envs_quat,
                    close_traj_point_2_w.reshape(1, 3).repeat(num_envs, 1), torch.tensor([1, 0, 0, 0], device=env.sim.device).reshape(1, 4).repeat(num_envs, 1)
                )
                close_traj_point_2_w_env = torch.cat([close_traj_point_2_pos, close_traj_point_2_quat], dim=-1)

                # place location to each robot coordinate
                close_traj_point_2_r_pos, close_traj_point_2_r_quat = math_utils.subtract_frame_transforms(
                    robot_base_w[:, :3], robot_base_w[:, 3:7],
                    close_traj_point_2_w_env[:, :3], close_traj_point_2_w_env[:, 3:7]
                )
                close_traj_point_2_r = torch.cat([close_traj_point_2_r_pos, close_traj_point_2_r_quat], dim=-1)

                # place location to each env coordinate
                close_traj_point_3_pos, close_traj_point_3_quat = math_utils.combine_frame_transforms(
                    envs_pos, envs_quat,
                    close_traj_point_3_w.reshape(1, 3).repeat(num_envs, 1), torch.tensor([1, 0, 0, 0], device=env.sim.device).reshape(1, 4).repeat(num_envs, 1)
                )
                close_traj_point_3_w_env = torch.cat([close_traj_point_3_pos, close_traj_point_3_quat], dim=-1)

                # place location to each robot coordinate
                close_traj_point_3_r_pos, close_traj_point_3_r_quat = math_utils.subtract_frame_transforms(
                    robot_base_w[:, :3], robot_base_w[:, 3:7],
                    close_traj_point_3_w_env[:, :3], close_traj_point_3_w_env[:, 3:7]
                )
                close_traj_point_3_r = torch.cat([close_traj_point_3_r_pos, close_traj_point_3_r_quat], dim=-1)
                ...
                camera_left_vis = env.scene["camera_left_vis"]

                camera_pos_left_vis, _ = math_utils.combine_frame_transforms(
                    robot_base_w[:, :3], robot_base_w[:, 3:7],
                    torch.tensor([camera_forward_vis, camera_to_base_vis, camera_height_vis], device=env.sim.device).reshape(1, 3).repeat(num_envs, 1), 
                    torch.tensor([1, 0, 0, 0], device=env.sim.device).reshape(1, 4).repeat(num_envs, 1)
                )

                camera_left_vis.set_world_poses_from_view(
                    camera_pos_left_vis.reshape(-1, 3),
                    camera_lookat.reshape(-1, 3)
                )
                camera_left_vis.reset()

                camera_right_vis = env.scene["camera_right_vis"]

                camera_pos_right_vis, _ = math_utils.combine_frame_transforms(
                    robot_base_w[:, :3], robot_base_w[:, 3:7],
                    torch.tensor([camera_forward_vis, -camera_to_base_vis, camera_height_vis], device=env.sim.device).reshape(1, 3).repeat(num_envs, 1), 
                    torch.tensor([1, 0, 0, 0], device=env.sim.device).reshape(1, 4).repeat(num_envs, 1)
                )

                camera_right_vis.set_world_poses_from_view(
                    camera_pos_right_vis.reshape(-1, 3),
                    camera_lookat.reshape(-1, 3)
                )
                camera_right_vis.reset()

                # set art object
                drawer = env.scene[table_object_name]
                joint_pos = drawer.data.default_joint_pos.clone()
                joint_vel = drawer.data.default_joint_vel.clone()
                # joint_pos = torch.tensor([0.26, 0.26, 0.26], device=env.sim.device).reshape(1, 3).repeat(num_envs, 1)
                # joint_pos = torch.tensor([0.26, 0.0, 0.0], device=env.sim.device).reshape(1, 3).repeat(num_envs, 1)
                joint_pos = torch.tensor([0.0, 0.0, 0.26], device=env.sim.device).reshape(1, 3).repeat(num_envs, 1)
                # joint_pos = torch.tensor([0.0, 0.26, 0.0], device=env.sim.device).reshape(1, 3).repeat(num_envs, 1)
            ...
            if iteration % T_grasp == T_init + 1:
                ...
                is_reach_close_goal_1_envs = []
                is_reach_close_goal_2_envs = []
                is_reach_close_goal_3_envs = []
                is_start_lift_envs = []
                ...
                T_start_place_envs = {}
                T_reach_place_envs = {}
                T_start_close_goal_1_envs = {}
                T_start_close_goal_2_envs = {}
                T_start_close_goal_3_envs = {}
                ...
                for env_i in range(num_envs):
                    ...
                    is_reach_close_goal_1_envs.append(False)
                    is_reach_close_goal_2_envs.append(False)
                    is_reach_close_goal_3_envs.append(False)
            ...
            if iteration % T_grasp > T_init:
                ...
                for env_i in range(num_envs):
                    ...
                    is_reach_close_goal_1 = is_reach_close_goal_1_envs[env_i]
                    is_reach_close_goal_2 = is_reach_close_goal_2_envs[env_i]
                    is_reach_close_goal_3 = is_reach_close_goal_3_envs[env_i]
                    ...
                    if not is_reach_grasp_up_goal:
                        ...
                        if is_reach_lift_up_goal:
                            ...
                            # target_to_ee_pos_robot_frame, _ = math_utils.combine_frame_transforms(
                            #     torch.zeros_like(robot_base_w[:, :3]), robot_base_w[:, 3:7],
                            #     target_to_ee_pos.reshape(1, 3), torch.tensor([1, 0, 0, 0], device=env.sim.device).reshape(1, 4)
                            # )
                        ...
                        if is_reach_place_goal:
                            ...
                            print(f"planning close traj 1 for env {env_i}")
                            close_traj_1_envs[env_i], _ = curobo_plan_traj(
                                motion_planner,
                                robot_qpos[env_i:env_i+1, :],
                                current_ee_pose[env_i:env_i+1, :3],
                                close_traj_point_1_r[env_i:env_i+1, :3],
                                current_ee_pose[env_i:env_i+1, 3:7],
                            )
                            T_start_close_goal_1_envs[env_i] = iteration

                    elif not is_reach_close_goal_1:
                        is_reach_close_goal_1 = is_ee_reach_goal(close_traj_point_1_r[env_i, :].reshape(-1), ee_frame_data, env_i, loc_success_threshold=0.08)
                        is_reach_close_goal_1_envs[env_i] = is_reach_close_goal_1
                        if is_reach_close_goal_1:
                            print(f"planning close traj 2 for env {env_i}")
                            close_traj_2_envs[env_i], _ = curobo_plan_traj(
                                motion_planner,
                                robot_qpos[env_i:env_i+1, :],
                                current_ee_pose[env_i:env_i+1, :3],
                                close_traj_point_2_r[env_i:env_i+1, :3],
                                current_ee_pose[env_i:env_i+1, 3:7],
                            )
                            T_start_close_goal_2_envs[env_i] = iteration
                    
                    elif not is_reach_close_goal_2:
                        is_reach_close_goal_2 = is_ee_reach_goal(close_traj_point_2_r[env_i, :].reshape(-1), ee_frame_data, env_i, loc_success_threshold=0.08)
                        is_reach_close_goal_2_envs[env_i] = is_reach_close_goal_2
                        if is_reach_close_goal_2:
                            print(f"planning close traj 3 for env {env_i}")
                            close_traj_3_envs[env_i], _ = curobo_plan_traj(
                                motion_planner,
                                robot_qpos[env_i:env_i+1, :],
                                current_ee_pose[env_i:env_i+1, :3],
                                close_traj_point_3_r[env_i:env_i+1, :3],
                                current_ee_pose[env_i:env_i+1, 3:7],
                            )
                            T_start_close_goal_3_envs[env_i] = iteration
                    
                    # elif not is_reach_close_goal_3:
                    #     is_reach_close_goal_3 = is_ee_reach_goal(close_traj_point_3_r[env_i, :].reshape(-1), ee_frame_data, env_i)
                    #     is_reach_close_goal_3_envs[env_i] = is_reach_close_goal_3
                    ...
                    if not is_reach_grasp_up_goal:
                        ...
                        T_start_close_goal_1 = T_start_close_goal_1_envs[env_i]
                        ee_goal = close_traj_1_envs[env_i][min(max(iteration - T_start_close_goal_1, 0), len(close_traj_1_envs[env_i]) - 1)]
                        gripper_switch = 1
                    elif not is_reach_close_goal_2:
                        T_start_close_goal_2 = T_start_close_goal_2_envs[env_i]
                        ee_goal = close_traj_2_envs[env_i][min(max(iteration - T_start_close_goal_2, 0), len(close_traj_2_envs[env_i]) - 1)]
                        gripper_switch = 1
                    elif not is_reach_close_goal_3:
                        T_start_close_goal_3 = T_start_close_goal_3_envs[env_i]
                        ee_goal = close_traj_3_envs[env_i][min(max(iteration - T_start_close_goal_3, 0), len(close_traj_3_envs[env_i]) - 1)]
                        gripper_switch = 1
            ...
            if iteration % T_grasp > T_init and current_data_idx < 20:
                camera_frame_envs = []
                camera_frame_envs_wall = []
                for env_i in range(min(num_envs, 8)):
                    camera_frame_envs.append(
                        np.concatenate([
                            obs_dict["policy"]["rgb_left_vis"][env_i].cpu().numpy(),
                            obs_dict["policy"]["rgb_right_vis"][env_i].cpu().numpy(),
                        ], axis=1)
                    )
                    camera_frame_envs_wall.append(
                        np.concatenate([
                            obs_dict["policy"]["rgb_out_vis_0"][env_i].cpu().numpy(),
                            obs_dict["policy"]["rgb_out_vis_1"][env_i].cpu().numpy(),
                            obs_dict["policy"]["rgb_out_vis_2"][env_i].cpu().numpy(),
                            obs_dict["policy"]["rgb_out_vis_3"][env_i].cpu().numpy(),
                        ], axis=1)
                    )
                camera_frame_envs = np.concatenate(camera_frame_envs, axis=0)
                camera_list.append(camera_frame_envs)
                camera_frame_envs_wall = np.concatenate(camera_frame_envs_wall, axis=0)
                camera_list_wall.append(camera_frame_envs_wall)

            if iteration % T_grasp == T_grasp - 1:
                if current_data_idx < 20:
                    ...
                    imageio.mimwrite(os.path.join(log_dir, "debug", f"{current_data_idx:0>3d}_cameras_wall.mp4"), camera_list_wall, fps=30)
                    print(f"saving {current_data_idx} data to video")
