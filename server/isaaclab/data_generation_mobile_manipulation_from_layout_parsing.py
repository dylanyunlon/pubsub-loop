...
parser.add_argument("--total_iterations_sim", type=int, default=2000, help="Number of episodes to store in the dataset.")
...
def main():
    ...
    for object_name in object_transform_dict:
        ...
        # mass_dict[object_name]["position"] = np.array([object_position["x"], object_position["y"], object_position["z"]]).reshape(3).tolist()
        # mass_dict[object_name]["rotation"] = np.array(R.from_euler("xyz", [object_rotation["x"], object_rotation["y"], object_rotation["z"]], degrees=True).as_quat(scalar_first=True)).reshape(4).tolist()
        ...
        while not collector_interface.is_stopped():
            ...
            if iteration % T_grasp == 0:
                ...
                for object_name in all_object_rigids:
                    ...
                    # if mass_dict[object_name]["static"]:
                    #     continue
            ...
            def initialize_pick_task(env_i, pick_object_id, pick_table_id):
                ...
                pick_ee_goals_up_after_envs[env_i][2] = torch.clamp(pick_ee_goals_up_after_envs[env_i][2], max=1.5)
                    ...
                    # Phase 3: Release object
                        ...
                        if is_reach_place_goal_up_envs[env_i] and not is_place_success_envs[env_i]:
                            ...
                            if torch.all(torch.abs(target_object_w_state_env_i_pos[:2] - place_location_w_env_i_pos[:2]) < min(CollisionCheckingConfig.MIN_DIST_TO_BOUNDARY, 0.1)) \
                                and iteration - reach_place_goal_T_envs[env_i] >= T_place:
                                ...
                                ee_frame_data_to_support_pos, ee_frame_data_to_support_quat = math_utils.subtract_frame_transforms(
                                    support_frame_data_pos[env_i].reshape(1, 3), support_frame_data_quat[env_i].reshape(1, 4),
                                    ee_frame_data_pos[env_i].reshape(1, -1), ee_frame_data_quat[env_i].reshape(1, -1)
                                )

                                ee_frame_data_to_support_pos[:, 0] = 0.5
                                ee_frame_data_pos_after_place, ee_frame_data_quat_after_place = math_utils.combine_frame_transforms(
                                    support_frame_data_pos[env_i].reshape(1, 3), support_frame_data_quat[env_i].reshape(1, 4),
                                    ee_frame_data_to_support_pos, ee_frame_data_to_support_quat,
                                )
                                ee_frame_data_pose_after_place_env_i = torch.cat([ee_frame_data_pos_after_place, ee_frame_data_quat_after_place], dim=-1)
                                
                                ee_frame_data_after_place_to_fixed_support_pos, ee_frame_data_after_place_to_fixed_support_quat = math_utils.subtract_frame_transforms(
                                    fixed_support_frame_data_pos[env_i].reshape(1, 3), fixed_support_frame_data_quat[env_i].reshape(1, 4),
                                    ee_frame_data_pose_after_place_env_i[:, :3], ee_frame_data_pose_after_place_env_i[:, 3:7]
                                )

                                ee_frame_data_pos_initial[env_i] = ee_frame_data_pos_after_place.clone().reshape(-1)
                                ee_frame_data_quat_initial[env_i] = ee_frame_data_quat_after_place.clone().reshape(-1)
                                fixed_support_frame_data_pos_initial[env_i] = fixed_support_frame_data_pos[env_i].clone()
                                fixed_support_frame_data_quat_initial[env_i] = fixed_support_frame_data_quat[env_i].clone()
                                support_frame_data_pos_initial[env_i] = support_frame_data_pos[env_i].clone()
                                support_frame_data_quat_initial[env_i] = support_frame_data_quat[env_i].clone()

                                ee_to_fixed_support_frame_pos_initial[env_i] = ee_frame_data_after_place_to_fixed_support_pos.clone().reshape(-1)
                                ee_to_fixed_support_frame_quat_initial[env_i] = ee_frame_data_after_place_to_fixed_support_quat.clone().reshape(-1)
                                ee_to_support_frame_pos_initial[env_i] = ee_frame_data_to_support_pos.clone().reshape(-1)
                                ee_to_support_frame_quat_initial[env_i] = ee_frame_data_to_support_quat.clone().reshape(-1)
            ...
            for env_i in range(num_envs):
                ...
                if current_phase == "complete":
                    ...
                    if env_i in pick_ee_goals_down_envs and env_i in robot_base_pick_pose_envs:
                        gripper_action = -1.0 if start_grasp_T_envs.get(env_i, 0) > T_hold * 0.8 else 1.0
                        pick_ee_goals_down_iter = reach_pick_down_traj_envs[env_i][min(max(iteration - reach_pick_down_T_envs[env_i], 0), len(reach_pick_down_traj_envs[env_i]) - 1)].reshape(1, 7)
