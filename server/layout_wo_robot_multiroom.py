            ...
            if room_id in material_descriptions:
                ...
                def repeat_texture(texture_map_pil, times):
                    for _ in range(times):
                        ...
                        texture_map_pil_np = texture_map_pil_np[int(0.01 * texture_map_pil_np.shape[0]):int(0.99 * texture_map_pil_np.shape[0]), int(0.01 * texture_map_pil_np.shape[1]):int(0.99 * texture_map_pil_np.shape[1])]
            ...
            for room in floor_plan.rooms:
                ...
                if room_id in material_descriptions:
                    ...
                    if isinstance(room_materials, dict) and 'floor' in room_materials and 'wall' in room_materials:
                        ...
                        validated_descriptions[room_id] = get_default_material_description(room.room_type)
                else:
                    # Room not found in response, use default
                    validated_descriptions[room_id] = get_default_material_description(room.room_type)
...
def get_default_material_description(room_type: str) -> Dict[str, str]:
    """
    Get default material descriptions based on room type.
    
    Args:
        room_type: The type of room
        
    Returns:
        Dictionary with 'floor' and 'wall' material descriptions
    """
    room_type_lower = room_type.lower()
    
    if 'bathroom' in room_type_lower or 'bath' in room_type_lower:
        return {
            'floor': 'ceramic tile for water resistance',
            'wall': 'moisture-resistant painted drywall'
        }
    elif 'kitchen' in room_type_lower:
        return {
            'floor': 'durable ceramic tile',
            'wall': 'painted drywall'
        }
    elif 'bedroom' in room_type_lower:
        return {
            'floor': 'warm hardwood flooring',
            'wall': 'painted drywall for comfort'
        }
    elif 'living' in room_type_lower or 'lounge' in room_type_lower:
        return {
            'floor': 'hardwood flooring for elegance',
            'wall': 'painted drywall for versatility'
        }
    elif 'office' in room_type_lower or 'study' in room_type_lower:
        return {
            'floor': 'professional carpet or hardwood',
            'wall': 'painted drywall'
        }
...
async def enrich_input_text_with_architectural_details(input_text: str) -> str:
    """
    Use Claude API to enrich the user's input text with detailed architectural specifications.
    This function expands basic descriptions into comprehensive layout requirements including:
    - Floor plan structure (single/multiple rooms)
    - Spatial distribution and connections between rooms
    - Room locations, sizes, and functions
    - Door and window placement specifications
    
    Args:
        input_text: Original user input describing desired layout
        
    Returns:
        Enriched text with detailed architectural specifications
    """
    try:
        # Check API key availability
        api_key = ANTHROPIC_API_KEY
        if not api_key:
            print("Warning: ANTHROPIC_API_KEY not available for input enrichment. Using original text.")
            return input_text
        
        # Create enrichment prompt
        enrichment_prompt = f"""You are an architectural consultant transforming basic room layout requests into detailed specifications.

USER REQUEST: {input_text.strip()}

TRANSFORM INTO DETAILED ARCHITECTURAL SPECIFICATION:

CONSTRAINTS:
- ALL ROOMS MUST BE RECTANGULAR with straight walls and 90° corners
- HALLWAYS ARE ROOMS: Treat as rectangular rooms with positions and connections
- Use qualitative size terms (large, compact, spacious) NOT exact dimensions

REQUIRED ELEMENTS:

1. LAYOUT STRUCTURE:
   - Single or multiple rooms
   - Overall pattern (linear, clustered, central corridor, open-plan)
   - Circulation strategy and room connections

2. ENTRANCE & CIRCULATION (if multiple rooms):
   - Main entrance location and connection point
   - Entry sequence (direct to main room vs. dedicated foyer)
   - Room-to-room connections via doors
   - Hallway rooms for complex circulation if needed

3. ROOM SPECIFICATIONS:
   - Each room: function, relative size, position, connections
   - Include hallway rooms with their own specs
   - Spatial relationships (adjacent, across from, etc.)

4. DOORS & WINDOWS:
   - Door placement for proper circulation
   - Window strategy for light and ventilation

EXAMPLES:
Single Room - Input: "single room home office"
Output: "ONLY ONE ROOM: Spacious rectangular home office with professional layout. Main entrance via standard door positioned for privacy from household traffic. Single large rectangular room with straight walls and 90° corners optimized for productivity. Two exterior windows on adjacent walls provide cross-ventilation and natural light throughout the day. Built-in storage along one wall maximizes floor space. Door placement allows easy access while maintaining work privacy."

Multi-Room - Input: "2 bedroom apartment" 
Output: "Modern 2-bedroom apartment with central hub circulation. Main entrance opens into living area entry zone. Living/dining area (medium rectangular room) serves as central hub connecting to: compact rectangular kitchen via wide opening, two medium rectangular bedrooms via doors, guest bathroom via door. Master bedroom connects to ensuite bathroom. Hub-and-spoke pattern maximizes efficiency and privacy. Each bedroom has exterior window, living area has large windows, kitchen has work area window."

Transform the request focusing on rectangular rooms, clear connections, and entrance strategy:"""
        ...
        if not enriched_text or len(enriched_text) < len(input_text):
            print("Warning: Enrichment produced shorter text than input. Using original text.")
            return input_text
        
        # Basic validation - ensure enriched text contains key architectural terms
        architectural_keywords = [
            'room', 'door', 'window', 'dimension', 'meter', 'layout', 
            'connect', 'position', 'size', 'space'
        ]
        
        enriched_lower = enriched_text.lower()
        keyword_count = sum(1 for keyword in architectural_keywords if keyword in enriched_lower)
        
        if keyword_count < 5:  # Require at least 5 architectural keywords
            print("Warning: Enriched text may not contain sufficient architectural detail. Using original text.")
            return input_text
        
        print(f"✅ Successfully enriched input text: {len(input_text)} → {len(enriched_text)} characters", file=sys.stderr)
        return enriched_text
        
    except anthropic.APIError as e:
        print(f"Warning: Claude API error during input enrichment: {e}. Using original text.")
        return input_text
    except Exception as e:
        print(f"Warning: Error during input enrichment: {e}. Using original text.")
        return input_text
...
def generate_scene_requirements(input_text: str, rooms_info: List[Dict] = None) -> Dict[str, str]:
    """
    Generate scene requirements from text description using Claude Sonnet 4.
    Compatible with both single-room and multi-room layouts.
    
    Args:
        input_text: The original user input describing the layout
        rooms_info: Optional list of room information dicts with 'id' and 'type' keys.
                   If provided, generates requirements for each room separately.
                   If None, generates requirements for a single room.
    
    Returns:
        Dictionary mapping room_id to scene_requirements string.
        For single rooms, returns {"default": scene_requirements_string}
    """
    global current_layout
    ...
    # Determine if we're dealing with single or multiple rooms
    is_multi_room = rooms_info is not None and len(rooms_info) > 1
    
    if is_multi_room:
        print(f"🏠 Multi-room layout detected: generating requirements for {len(rooms_info)} rooms", file=sys.stderr)
        # Generate requirements for each room separately
        all_room_requirements = {}
        
        for room_info in rooms_info:
            room_id = room_info.get('id', 'unknown')
            room_type = room_info.get('type', 'room')
            room_dimensions = room_info.get('dimensions', 'medium-sized')
            
            print(f"  📝 Generating requirements for {room_type} ({room_id})...", file=sys.stderr)
            
            # Generate scene requirements for this specific room
            room_requirement = _generate_single_room_requirements(
                input_text, 
                room_type, 
                room_dimensions
            )
            
            all_room_requirements[room_id] = room_requirement
            print(f"  ✅ Generated {len(room_requirement)} characters for {room_type}", file=sys.stderr)
        
        print(f"✅ All scene requirements generated for {len(rooms_info)} rooms", file=sys.stderr)
        return all_room_requirements
    
    else:
        # Single room - use original logic
        print(f"🏠 Single room layout: generating requirements", file=sys.stderr)
        single_room_type = rooms_info[0].get('type', 'room') if rooms_info else None
        single_room_dimensions = rooms_info[0].get('dimensions', 'medium-sized') if rooms_info else 'medium-sized'
        
        scene_requirement = _generate_single_room_requirements(
            input_text,
            single_room_type,
            single_room_dimensions
        )
        
        return {"default": scene_requirement}


def _generate_single_room_requirements(input_text: str, room_type_hint: str = None, room_dimensions_hint: str = None) -> str:
    """
    Generate scene requirements for a single room.
    
    Args:
        input_text: The original user input
        room_type_hint: Optional hint about the room type
        room_dimensions_hint: Optional hint about room dimensions
    
    Returns:
        Scene requirements string for the room
    """
    scene_requirements = ""
    ...
    # Build the prompt with room-specific hints if available
    room_context = ""
    if room_type_hint:
        room_context += f"\nROOM TYPE HINT: This is a {room_type_hint}."
    if room_dimensions_hint:
        room_context += f"\nROOM SIZE HINT: The room dimensions are approximately {room_dimensions_hint}."
    
    prompt = f"""You are an interior design expert. Based on the room description, generate comprehensive scene requirements.

ROOM DESCRIPTION: {input_text}{room_context}
...
async def generate_room_layout(input_text: str) -> str:
    ...
    try:
        # Stage 0: Enrich the input text with Claude API to get more architectural details
        # print("🔍 Stage 0: Enriching input text with architectural details...", file=sys.stderr)
        # enriched_input_text = await enrich_input_text_with_architectural_details(input_text)
        
        # # Use enriched text for all subsequent stages
        # processing_text = enriched_input_text

        # Stage 1: Generate room structure first (needed to know how many rooms)
        room_structure_result = await generate_room_structure(input_text)
        ...
        # Stage 1.5: Generate scene requirements based on room structure
        print("🎨 Stage 1.5: Generating scene requirements for rooms...", file=sys.stderr)
        rooms_info = [
            {
                'id': room.id,
                'type': room.room_type,
        ...
        scene_requirements_dict = generate_scene_requirements(input_text, rooms_info)
        print(f"✅ Scene requirements generated for {len(scene_requirements_dict)} room(s)", file=sys.stderr)
        ...
        # # Auto-correct door/window integrity issues if found
        # door_window_correction_method = "no_correction_needed"
        # if not door_window_integrity["valid"]:
        #     print(f"Door/window integrity issues found: {door_window_integrity['total_issues']} issues. Attempting automatic correction...", file=sys.stderr)
        #     print("door_window_integrity: ", door_window_integrity, file=sys.stderr)
        #     try:
        #         # Apply automatic door/window correction
        #         correction_result = correct_door_window_integrity(rooms_data_for_check)
                
        #         if correction_result["success"] and correction_result["guaranteed_integrity"]:
        #             # Update the current layout with corrected data
        #             corrected_layout_data = {
        #                 "building_style": current_layout.building_style,
        #                 "rooms": correction_result["corrected_rooms_data"]
        #             }
                    
        #             # Parse corrected layout into floor plan
        ...
        #             # Preserve metadata but update ID
        #             corrected_floor_plan.id = f"{old_layout_id}"
        #             corrected_floor_plan.description = current_layout.description
                    
        #             # Update global layout
        #             current_layout = corrected_floor_plan
                    
        #             # Update integrity status
        #             door_window_integrity = correction_result["final_integrity"]
        #             door_window_correction_method = "automatic_correction"
                    
        #             print(f"Door/window integrity automatically corrected: {correction_result['correction_summary']['total_corrections']} corrections applied", file=sys.stderr)
        #         else:
        #             door_window_correction_method = "correction_failed"
        #             print("Automatic door/window correction failed or incomplete", file=sys.stderr)
        #     except Exception as e:
        #         door_window_correction_method = "correction_failed"
        #         print(f"Door/window correction failed: {e}", file=sys.stderr)
        
        # Stage 5: Final validation
        # print("🏗️ Stage 5: Final validation...", file=sys.stderr)
        # final_validation_result = await validate_current_layout()
        # final_validation_data = json.loads(final_validation_result)
        ...
        # Store scene requirements in policy_analysis (supports both single and multi-room)
        policy_analysis["scene_requirements"] = scene_requirements_dict
        ...
        summary = {
            ...
            "scene_recommendations": scene_requirements_dict,
        }
...
@mcp.tool()
async def get_layout_from_json(json_file_path: str = "") -> str:
    """
...
3. "replace" - Completely replacing all objects with new ones (keywords: "replace all", "new layout", "start fresh", or general placement without mentioning existing objects)
...
```json
{{"operation_type": "replace", "analysis": "Complete room redesign"}}
```
    ...
    try:
        ...
        try:
            ...
            if "operation_type" not in analysis_result:
                analysis_result["operation_type"] = "replace"
            ...
            elif any(keyword in placement_conditions.lower() for keyword in ["remove", "delete", "take away", "get rid of"]):
                return {"operation_type": "remove", "analysis": "Detected remove operation from keywords", "objects_to_remove": []}
            else:
                return {"operation_type": "replace", "analysis": "Default to replace operation", "objects_to_remove": []}
    
    except Exception as e:
        print(f"Error in analyze_placement_operation: {e}")
        return {"operation_type": "replace", "analysis": f"Error during analysis: {e}", "objects_to_remove": []}
...
# @mcp.tool()
async def place_objects_in_room(room_id: str = "", placement_conditions: str = ""):
    """
    OPERATION MODES:
...
Focus on physical details for 3D generation."""
            ...
            response_text = response.content[0].text.strip()
            print(f"✅ Claude API response received ({len(response_text)} characters)", file=sys.stderr)
...
# @mcp.tool()
async def move_one_object_with_condition_in_room(room_id: str = "", condition: str = ""):
    """
...
@mcp.tool()
async def place_objects_in_room_multiroom(room_conditions: List[Dict[str, str]]) -> str:
    """
    Place objects in multiple rooms simultaneously in parallel.
    
    This tool executes place_objects_in_room operations on multiple rooms at the same time,
    which is much more efficient than calling place_objects_in_room sequentially for each room.
    All room operations run in parallel and return when all are complete.
    
    OPERATION MODES (same as place_objects_in_room):
    1. ADD MODE: Add multiple new objects while keeping existing ones
       - Triggered by: "add furniture", "place additional items", "include more objects"
       - Combines existing objects with new recommendations
    ...
    WHEN NOT TO USE:
    - Adding/moving just one specific object per room (use add_one_object_with_condition_in_room)
    - Moving existing objects (use move_one_object_with_condition_in_room_multiroom)
    - Precise single-object placement requirements
    - wanted to reorganize the existing furniture, not replace it with completely new items

    After each operation, it uses semantic and physics critic to analyze the room design quality and get improvement suggestions.
    
    Args:
        room_conditions: List of dictionaries, each containing:
            - "room_id": The ID of the room to furnish/modify
            - "condition": Text describing the operation and requirements
            
    CONDITION FORMAT AND EXAMPLES (for each room):
        ...
        [
            {
                ...
                "condition": "add essential furniture for a teenager bedroom: desk against wall, chair facing desk, bed in corner"
            },
            {
                "room_id": "room_87654321", 
                "condition": "design a modern minimalist living room with gray sofa against wall, glass coffee table in front of sofa"
            },
            {
                "room_id": "room_11111111",
                "condition": "remove the old armchair and side table"
            }
        ]
    ...
    Returns:
        ...
        - total_rooms: Number of rooms processed
        - successful_rooms: Count of successfully processed rooms
        - failed_rooms: Count of failed room operations
        - results: List of individual results from each room operation, each containing:
            * List of objects placed/removed/replaced
            * Operation type performed (add/remove/replace)
            * Summary of changes made to the room
            * Details about object selection and placement reasoning
            * Physics critic analysis results
            * Semantic critic analysis results
    """
    global current_layout
    
    print(f"🔧 Starting place_objects_in_room_multiroom for {len(room_conditions)} rooms", file=sys.stderr)
    ...
    if not room_conditions or len(room_conditions) == 0:
        return json.dumps({
            "success": False,
            "error": "room_conditions parameter is required and must contain at least one room"
        })
    
    # Validate input format
    for i, item in enumerate(room_conditions):
        if not isinstance(item, dict):
            return json.dumps({
                "success": False,
                "error": f"Item {i} in room_conditions must be a dictionary with 'room_id' and 'condition' keys"
            })
        if "room_id" not in item or "condition" not in item:
            return json.dumps({
                "success": False,
                "error": f"Item {i} in room_conditions must contain both 'room_id' and 'condition' keys"
            })
    
    try:
        # Execute all operations in parallel
        print(f"🚀 Executing {len(room_conditions)} place_objects_in_room operations in parallel...", file=sys.stderr)
        tasks = []
        for item in room_conditions:
            room_id = item.get("room_id", "")
            condition = item.get("condition", "")
            print(f"  📝 Room {room_id}: {condition[:100]}...", file=sys.stderr)
            tasks.append(place_objects_in_room(room_id, condition))
        
        # Wait for all operations to complete
        results = await asyncio.gather(*tasks, return_exceptions=True)
        print(f"✅ All {len(room_conditions)} operations completed", file=sys.stderr)
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                print(f"❌ Room {room_conditions[i].get('room_id')} failed with exception: {str(result)}", file=sys.stderr)
                processed_results.append({
                    "room_id": room_conditions[i].get("room_id", "unknown"),
                    "success": False,
                    "error": str(result)
                })
                failed_count += 1
                ...
                try:
                    ...
                    if parsed_result.get("success", False):
                        successful_count += 1
                        print(f"✅ Room {room_conditions[i].get('room_id')} completed successfully", file=sys.stderr)
                    else:
                        failed_count += 1
                        print(f"⚠️ Room {room_conditions[i].get('room_id')} completed with errors", file=sys.stderr)
                except json.JSONDecodeError:
                    print(f"❌ Room {room_conditions[i].get('room_id')} returned invalid JSON", file=sys.stderr)
                    processed_results.append({
                        "room_id": room_conditions[i].get("room_id", "unknown"),
                        "success": False,
        ...
        print(f"📊 Summary: {successful_count} successful, {failed_count} failed out of {len(room_conditions)} total", file=sys.stderr)
        
        # Collect all objects per room
        all_objects_per_room = {}
        for room in current_layout.rooms:
            all_objects_per_room[room.id] = get_object_description_list(room.objects)
        
        return json.dumps({
            "success": True,
            "total_rooms": len(room_conditions),
            "successful_rooms": successful_count,
            "failed_rooms": failed_count,
            "all_objects_per_room": all_objects_per_room,
            "results": processed_results,
        }, indent=2)
        
    except Exception as e:
        print(f"❌ Unexpected error in place_objects_in_room_multiroom: {str(e)}", file=sys.stderr)
        return json.dumps({
            "success": False,
            "error": f"Unexpected error in place_objects_in_room_multiroom: {str(e)}"
        })

@mcp.tool()
async def move_one_object_with_condition_in_room_multiroom(room_conditions: List[Dict[str, str]]) -> str:
    """
    Move objects in multiple rooms simultaneously in parallel.
    
    This tool executes move_one_object_with_condition_in_room operations on multiple rooms at the same time,
    which is much more efficient than calling move_one_object_with_condition_in_room sequentially for each room.
    All room operations run in parallel and return when all are complete.
    
    After each movement, it uses semantic and physics critic to analyze the room design quality and get improvement suggestions.
    
    USE THIS WHEN:
    - Moving specific objects to new locations in multiple rooms (floor, wall, or on top of another object)
    - Repositioning existing furniture with spatial constraints across rooms
    - Reorienting objects with specific facing directions in multiple rooms
    - Adjusting object placement relative to other objects or room features in multiple rooms
    
    Note: Each condition must follow the format specified below with object_id and location
    
    DO NOT USE for:
    - Adding new objects (use place_objects_in_room_multiroom instead)
    - Removing objects (use place_objects_in_room_multiroom with REMOVE mode)
    - Complete room reorganization (use other layout tools)
    ...
    Each room operation:
    - Moves exactly ONE object per room
    - Supports floor, wall, or on-top-of-object placement
    - Includes intelligent condition analysis and collision avoidance
    
    Args:
        room_conditions: List of dictionaries, each containing:
            - "room_id": The ID of the room containing the object to move
            - "condition": Movement instruction that MUST follow the format below
            
    [Important] CONDITION FORMAT AND EXAMPLES (for each room):
    ...
    Components:
    - object_type: Type of object to move (e.g., chair, desk, sofa)
    - object_id: Unique identifier of the object (MUST be exact ID from room)
    - location: Must be one of: 'floor', 'wall', or an existing object_id
    - spatial_relationships: Positioning relative to other objects or wall (window and door not supported)
      Preferred relative relationship with other objects: near/far, in front/side/left/right/around, center aligned, face to/face same as, on top of, above. (other relationships are hard to parse in the tool)
    ...
    4. Move picture (object_id: picture_004) to wall, above the sofa.
    5. Move sofa (object_id: sofa_005) to floor, facing the coffee table.
    6. Move table (object_id: table_006) to floor, near the window and left of the bed.
            
    Example input:
        [
            {
                "room_id": "room_12345678",
                "condition": "Move chair (object_id: chair_001) to floor, facing and aligned with the desk_002"
            },
            {
                "room_id": "room_87654321",
                "condition": "Move lamp (object_id: lamp_003) to on top of desk_002"
            },
            {
                "room_id": "room_11111111",
                "condition": "Move sofa (object_id: sofa_005) to floor, against the wall and facing coffee_table_001"
            }
        ]
    ...
    Returns:
        ...
        - failed_rooms: Count of failed room operations
        - results: List of individual results from each room operation, each containing:
            * Movement result and object details
            * Old and new position/rotation
            * Child objects removed information (if any objects were on top)
            * Condition analyzed
            * Physics critic analysis results
            * Semantic critic analysis results
    """
    ...
    print(f"🔧 Starting move_one_object_with_condition_in_room_multiroom for {len(room_conditions)} rooms", file=sys.stderr)
    ...
    try:
        # Execute all operations in parallel
        print(f"🚀 Executing {len(room_conditions)} move_one_object_with_condition_in_room operations in parallel...", file=sys.stderr)
        tasks = []
        for item in room_conditions:
            room_id = item.get("room_id", "")
            condition = item.get("condition", "")
            print(f"  📝 Room {room_id}: {condition[:100]}...", file=sys.stderr)
            tasks.append(move_one_object_with_condition_in_room(room_id, condition))
        ...
        print(f"❌ Unexpected error in move_one_object_with_condition_in_room_multiroom: {str(e)}", file=sys.stderr)
        return json.dumps({
            "success": False,
            "error": f"Unexpected error in move_one_object_with_condition_in_room_multiroom: {str(e)}"
        })
...
async def room_semantic_critic(
    ...
    try:
        ...
        # existing_objects_description = get_object_description_list(room.objects, add_object_descendants=True)
        # existing_objects_description = [f"Object {i+1}: {desc}" for i, desc in enumerate(existing_objects_description)]
        # existing_objects_description = "\n".join(existing_objects_description)
        # existing_objects_description = dict2str(get_object_description_list_with_relation(room.objects))
...
"""
        ...
        if "object_addition_analysis" in claude_analysis:
            ...
            if "object_combos_analysis" in addition_analysis:
                for combo_item in addition_analysis["object_combos_analysis"]:
                    ...
                    # combo_action = {
                    #     "type": "object_combo",
                    #     "object_id": combo_item.get("object_id", ""),
                    #     "possible_object_combos": combo_item.get("possible_object_combos", []),
                    #     "priority": priority,
                    #     "suggested_operation": {
                    #         "type": "ADD",
                    #         "operation_guidance": "call place_objects_in_room(room_id, condition) to add the object",
                    #     }
                    # }
                    combo_action = {
                    }
        ...
        # modification_actions = []
        # for action in result["next_step"]["actions"]:
        #     operation_type = action.get("suggested_operation", {}).get("type", "")
        #     if operation_type in ["REMOVE", "REPLACE", "MOVE"]:
        #         modification_actions.append(action)
        
        # Sort modification actions by score (descending)
        # modification_actions.sort(key=lambda x: x.get("score", 0), reverse=True)
        
        # Apply max_recommendations limit only to modification actions
        ...
        # Sort by priority (for additions) or score (for modifications), both in descending order
        # result["next_step"]["actions"].sort(key=lambda x: x.get("priority", x.get("score", 0)), reverse=True)
