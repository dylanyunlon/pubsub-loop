...
if __name__ == "__main__":
    ...
    # Load SBERT features instead of CLIP features
    with open(os.path.join(partnet_root_dir, "sbert_features.pkl"), "rb") as f:
        data = pickle.load(f)
    sbert_features = data["text_features"]
    ...
    pca = PCA(n_components=min(50, sbert_features.shape[1]))  # Reduce to 50 dimensions or less if features are smaller
    sbert_features_pca = pca.fit_transform(sbert_features)
    print(f"PCA reduced features shape: {sbert_features_pca.shape}")
    ...
    if umap is not None:
        ...
        features_2d = reducer.fit_transform(sbert_features_pca)
        method_name = "UMAP"
    else:
    ...
    plt.title(f'PartNet Objects - SBERT Features Visualization (PCA + {method_name}) - {color_scheme} colors')
    ...
    vis_save_path = os.path.join(partnet_root_dir, f"sbert_features_visualization_{method_name.lower()}.png")
