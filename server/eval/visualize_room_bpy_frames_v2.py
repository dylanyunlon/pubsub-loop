...
def find_ray_wall_intersection(room_center, angle, room):
    """
    Find the intersection point of a ray from room center at given angle with room walls.
    
    Args:
        room_center: numpy array [x, y, z] of room center
        angle: angle in radians
        room: Room object with walls
        
    Returns:
        numpy array [x, y] of intersection point, or None if no intersection found
    """
    ray_origin = room_center[:2]  # x, y only
    ...
    for wall in room.walls:
        ...
        # Solve: ray_origin + t * ray_dir = wall_start + s * wall_vec
        # This gives us a 2x2 system
        denominator = ray_dir[0] * wall_vec[1] - ray_dir[1] * wall_vec[0]
        
        if abs(denominator) < 1e-10:  # Parallel
            continue
        
        diff = wall_start - ray_origin
        t = (diff[0] * wall_vec[1] - diff[1] * wall_vec[0]) / denominator
        s = (diff[0] * ray_dir[1] - diff[1] * ray_dir[0]) / denominator
        
        # Check if intersection is valid (ray goes forward, point is on wall segment)
        if t > 0 and 0 <= s <= 1:
...
def render_room_circular_video(layout: FloorPlan, room_id: str, output_path: str, 
    ...
    # Calculate camera trajectory radius (ensure whole room is visible)
    radius = max(room.dimensions.width, room.dimensions.length) * radius_scale
    # Camera lifted much higher above the room for elevated perspective
    camera_height = room.dimensions.height * (1.0 + max(room.dimensions.width, room.dimensions.length) * 0.45)
    ...
    camera.data.angle = np.radians(60)  # 60 degree FOV (narrower for more zoom)
    ...
    for frame_idx in tqdm(range(num_frames), desc="Rendering frames", unit="frame"):
        ...
        # Find intersection with wall at this angle
        intersection_point = find_ray_wall_intersection(room_center, angle, room)
        
        if intersection_point is not None:
            # Place camera above the wall intersection point
            camera_x = intersection_point[0]
            camera_y = intersection_point[1]
        else:
            # Fallback to circular trajectory if no intersection found
        ...
        # Camera looks down at room center from above (looking at floor level)
        lookat_pos = np.array([room_center[0] - radius * 0.05 * np.cos(angle), room_center[1] - radius * 0.05 * np.sin(angle), room_position[2]])
        
        # Update camera position and orientation
        setup_camera_look_at(camera, camera_pos, lookat_pos)
        
        # Hide walls based on camera view direction (disabled - keep all walls visible)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--angle-step", type=int, default=45, help="Degrees between frames (default: 45)")
    ...
    try:
        ...
        if not args.room_id.startswith("room_"):
            args.room_id = layout.rooms[0].id
