...
def create_bbox_objects(room):
    ...
    for obj in room.objects:
        ...
        rotation_x = np.radians(obj.rotation.x)
        rotation_y = np.radians(obj.rotation.y)
        ...
        original_center = mathutils.Vector((0, 0, height/2))
        center = mathutils.Vector((obj_x, obj_y, obj_z))
        scale_matrix = mathutils.Matrix.Diagonal((width, length, height)).to_4x4()
        rotation_matrix = mathutils.Matrix.Rotation(rotation_z, 4, 'Z') @ mathutils.Matrix.Rotation(rotation_y, 4, 'Y') @ mathutils.Matrix.Rotation(rotation_x, 4, 'X')
        translation_matrix = mathutils.Matrix.Translation(center)
        original_translation_matrix = mathutils.Matrix.Translation(original_center)
        
        bbox_obj.matrix_world = translation_matrix @ rotation_matrix @ original_translation_matrix @ scale_matrix
...
async def test_load_layout(layout_id: str):
    ...
    try:
        ...
        preview_save_dir = os.path.join(scene_save_dir, "top_down")
