...
def render_physics_simulation(layout: FloorPlan, room_id: str, traced_data_all: dict,
    ...
    temp_dir = os.path.join(os.path.dirname(output_path), "temp_frames_phy")
    ...
    for frame_idx in tqdm(range(num_frames), desc="Rendering frames", unit="frame"):
        ...
        if camera_mode == "circular":
            ...
            # Fixed camera position looking at room
...
if __name__ == "__main__":
    ...
    # Create output path
    output_path = os.path.join(layout_path, f"{room_id}_physics_simulation.mp4")
