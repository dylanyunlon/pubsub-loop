...
def get_task_definition_text(
    room_type: str,
    pre_generated_object_info_dict_path: str,
    pre_generated_objects_description: str
):
    return f"""
Task: Generate a medium-sized {room_type}.

Special Requirements: You need to reuse some pregenerated object assets in your generation. Those are required objects in the room and it is a must to place those objects into the room (Highest priority).

Pre-generated object information dictionary path: {pre_generated_object_info_dict_path}

Pre-generated objects: {pre_generated_objects_description}
...
[CRITICAL] You need to mention the pre-generated objects as well in the field of required_object_descriptions, including the object type, name, and description, and placements.
...
--- 2.0 PRE-GENERATED OBJECT PLACEMENT ---

[CRITICAL] Always place pre-generated object in the first time of object placement.
[CRITICAL] Those pre-generated objects have the highest priority to place into the room.
[CRITICAL] You need to use the **pre-generated object information dictionary path** in your object placement tool use.

[CRITICAL] DO NOT place pregenerated solely in one placement tool call! Placing solely without context would destroy the room layout appearance since no anchor context objects for those pregenerated objects in placing.
...
async def main():
    ...
    parser.add_argument(
        '--base_layout_dict_path',
        type=str,
        required=True,
        help='Base layout dictionary path'
    )

    parser.add_argument(
        '--from_task_required_objects',
        action='store_true',
        help='Whether to use task required objects from task description'
    )
    ...
    parser.add_argument(
        ...
        nargs='*',
        default=None,
        help='Optional list of object ids to keep if not from task required objects'
    )
    ...
    base_layout_dict_path = args.base_layout_dict_path
    from_task_required_objects = args.from_task_required_objects
    object_ids_to_keep = args.object_ids_to_keep

    layout_dir = os.path.dirname(base_layout_dict_path)

    assert os.path.exists(base_layout_dict_path), f"Base layout dictionary path not found: {base_layout_dict_path}"
    with open(base_layout_dict_path, "r") as f:
        base_layout_dict = json.load(f)

    base_room = base_layout_dict["rooms"][0]
    base_room_objects = base_room["objects"]
    
    objects_to_keep = []

    if from_task_required_objects:
        assert "policy_analysis" in base_layout_dict, "Policy analysis not found in base layout dictionary"
        assert "minimum_required_objects" in base_layout_dict["policy_analysis"], "Minimum required objects not found in policy analysis"


        for obj_dict in base_layout_dict["policy_analysis"]["minimum_required_objects"]:
            ...
            for matched_object_id, matched_object_type in zip(matched_object_ids, matched_object_types):
                matched_object = next((obj for obj in base_room_objects if obj["id"] == matched_object_id), None)
                assert matched_object is not None, f"Matched object not found: {matched_object_id}"
                if matched_object["place_id"] in ["floor", "wall"]:
                    matched_object["place_object_type"] = matched_object["place_id"]
                else:
                    matched_placed_object = next((obj for obj in base_room_objects if obj["id"] == matched_object["place_id"]), None)
                    assert matched_placed_object is not None, f"Matched placed object not found: {matched_object['place_id']}"
                    matched_object["place_object_type"] = matched_placed_object["type"]
                objects_to_keep.append(matched_object)
                

    else:
        assert object_ids_to_keep is not None, "Object ids to keep must be provided if not from task required objects"
        for object_id in object_ids_to_keep:
            matched_object = next((obj for obj in base_room_objects if obj["id"] == object_id), None)
            assert matched_object is not None, f"Matched object not found: {object_id}"
            if matched_object["place_id"] in ["floor", "wall"]:
                matched_object["place_object_type"] = matched_object["place_id"]
            else:
                matched_placed_object = next((obj for obj in base_room_objects if obj["id"] == matched_object["place_id"]), None)
                assert matched_placed_object is not None, f"Matched placed object not found: {matched_object['place_id']}"
                matched_object["place_object_type"] = matched_placed_object["description"]

            objects_to_keep.append(matched_object)

    scene_aug_info_dict = {
        "id": base_layout_dict["id"],
        "selected_objects": objects_to_keep
    }

    scene_aug_info_path = os.path.join(layout_dir, "scene_aug_info.json")
    with open(scene_aug_info_path, "w") as f:
        json.dump(scene_aug_info_dict, f, indent=4)
    scene_aug_info_path = os.path.abspath(scene_aug_info_path)

    print(f"Scene aug info saved to: {scene_aug_info_path}")

    room_type = base_room["room_type"]
    objects_to_keep_description = "; ".join([f"An object of [{obj['description']}] (id: {obj['id']}) placed on [{obj['place_object_type']}] (id: {obj['place_id']})" for obj in objects_to_keep])

    text_instruction = get_task_definition_text(room_type, scene_aug_info_path, objects_to_keep_description)

    task_description_dict = {
        "room_type": room_type,
        "objects_to_keep_description": objects_to_keep_description,
        "scene_aug_info_path": scene_aug_info_path,
        "task_definition_text": text_instruction,
        "finished": False
    }
    
    task_desc_head = text_instruction.strip(" \n\t").replace(' ', '_').replace('.', '_').replace('/', '_')[:20]
    label_id = f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{task_desc_head}"
    save_dir = "./scene_aug_descs"
    ...
    if not text_instruction:
        print(f"❌ Error: Instruction is empty: {text_instruction}")
        sys.exit(1)
