...
async def main():
    ...
    input_prompt_head = input_prompt.strip(" \n\t").replace(' ', '_').replace('.', '_').replace('/', '_')[:20]
    label_id = f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{input_prompt_head}"
    save_dir = "./input_prompts_source"
