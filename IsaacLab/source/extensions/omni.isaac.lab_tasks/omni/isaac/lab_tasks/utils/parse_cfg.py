...
def load_cfg_from_registry(
    ...
    if isinstance(cfg_entry_point, str) and cfg_entry_point.endswith(".yaml"):
        if os.path.exists(cfg_entry_point):
            ...
            print(f"mod_name: {mod_name}; file_name: {file_name}")

            if "class" in mod_name:
                class_path = mod_name.split("'")[1]
                module_path, class_name = class_path.rsplit('.', 1)
                module = importlib.import_module(module_path)
                cfg_entry_point = getattr(module, class_name)

                print(f"cfg_entry_point: {cfg_entry_point}; loaded from: {config_yaml}")

                cfg_cls = cfg_entry_point(config_yaml=config_yaml)
