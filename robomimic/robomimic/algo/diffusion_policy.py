...
class DiffusionPolicyUNet(PolicyAlgo):
    ...
    def to_device(self, device):
        ...
        # Move EMA model if it exists
        # EMAModel has an averaged_model attribute that needs to be moved
        if self.ema is not None:
    ...
    def _get_action_trajectory(self, obs_dict, goal_dict=None):
        ...
        for k in list(inputs["obs"].keys()):
            if k not in self.obs_shapes:
                inputs["obs"].pop(k)
        for k in self.obs_shapes:
            # first two dimensions should be [B, T] for inputs
            if inputs["obs"][k].ndim - 1 == len(self.obs_shapes[k]):
                inputs["obs"][k] = inputs["obs"][k].unsqueeze(0)
            # print("\n\n", k, inputs["obs"][k].shape, self.obs_shapes[k])
            # assert inputs["obs"][k].ndim - 2 == len(self.obs_shapes[k])
