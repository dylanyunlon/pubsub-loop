...
def get_s3_credentials():
    ...
    if not access_key or not secret_key:
        ...
        print("You can also try using default AWS credentials (IAM role, ~/.aws/credentials, etc.)")
        return None, None, region
    
    return access_key, secret_key, region

def connect_s3():
    print("connecting to S3 filesystem...")
    
    access_key, secret_key, region = get_s3_credentials()
...
def handle_s3(s3_path):
    global s3_fs
    if s3_fs is None or not s3_path.startswith("s3://"):
        return s3_path
    else:
        return s3_fs.open(s3_path.replace("s3://", ""), "rb")
...
def get_demos_for_filter_key(hdf5_path, filter_key):
    ...
    Returns:
        demo_keys ([str]): list of demonstration keys that
            ...
            "demo_1"].
    """
    f = h5py.File(handle_s3(hdf5_path), "r")
...
def get_env_metadata_from_dataset(dataset_path, set_env_specific_obs_processors=True):
    ...
    f = h5py.File(handle_s3(dataset_path), "r")
