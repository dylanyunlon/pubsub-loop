...
def run_rollout(
    ...
    try:
        for step_i in range(horizon):
            ...
            # compute reward
            total_reward += float(r.cpu().reshape(-1))
...
def run_epoch(model, data_loader, epoch, validate=False, num_steps=None, obs_normalization_stats=None, data_loader_iter=None):
    ...
    Args:
        ...
        data_loader_iter (iterator or None): if provided, use this iterator instead of creating a new one
            from data_loader. This can improve performance for large datasets.
