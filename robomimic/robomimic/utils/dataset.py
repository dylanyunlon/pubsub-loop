...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def hdf5_file(self):
        ...
        if self._hdf5_file is None:
            # Disable SWMR for S3 files since S3 VFD doesn't support SWMR mode
            use_swmr = self.hdf5_use_swmr and not self.hdf5_path.startswith("s3://")
            self._hdf5_file = h5py.File(FileUtils.handle_s3(self.hdf5_path), 'r', swmr=use_swmr, libver='latest')
    ...
    def get_dataset_for_ep(self, ep, key):
        ...
        if key_should_be_in_memory:
            ...
            try:
                ...
                print(f"Key {ep} {key} not found in dataset, returning None")
                ret = None
        return ret
