...
class EnvGym(EB.EnvBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        env_init_config_yaml = kwargs.get("env_init_config_yaml", None)
        # env_init_config_yaml = "/home/hongchix/codes/simgen-mcp/server/logs/robomimic/Isaac-Lift-Single-Obj-Scene-Franka-IK-Rel-v0/params/env_init.yaml"

        env_cfg = parse_env_cfg(
            env_name, 
            device="cuda", 
            num_envs=1, 
            config_yaml=env_init_config_yaml
        )
    ...
    def render(self, mode="human", height=None, width=None, camera_name=None, **kwargs):
        ...
        """
        if mode =="human":
            return self.env.unwrapped.render(mode=mode, **kwargs)
        if mode == "rgb_array":
    ...
    def get_observation(self, obs=None):
        ...
        for obs_term in obs[0]["policy"]:
            obs[0]["policy"][obs_term] = obs[0]["policy"][obs_term][0]
        return obs[0]["policy"]
