...
def train(config, device, resume=False):
    ...
    for dataset_cfg in config.train.data:
        ...
        if not dataset_path.startswith("s3://") and not os.path.exists(dataset_path):
    ...
    # create data loader iterators once for better performance with large datasets
    train_loader_iter = iter(train_loader)
    valid_loader_iter = iter(valid_loader) if valid_loader is not None else None
    ...
    for epoch in range(start_epoch, (config.train.num_epochs + 1) if config.train.end_epoch is None else (config.train.end_epoch + 1)):
        ...
        if config.experiment.validate:
            with torch.no_grad():
                step_log, valid_loader_iter = TrainUtils.run_epoch(model=model, data_loader=valid_loader, epoch=epoch, validate=True, num_steps=valid_num_steps, data_loader_iter=valid_loader_iter)
            for k, v in step_log.items():
                if k.startswith("Time_"):
...
def main(args):
    ...
    if args.dataset is not None:
        config.train.data = [{"path": d} for d in args.dataset]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help="(optional) if provided, override the dataset path(s) defined in the config. Can provide multiple dataset paths separated by spaces",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="set this flag to end training at specific epoch",
    )

    args = parser.parse_args()

    # args = argparse.Namespace()
    # args.config = "robomimic/exps/templates/bc_depth.json"
    # args.dataset = "/home/hongchix/codes/simgen-mcp/server/logs/robomimic/Isaac-Lift-Single-Obj-Scene-Franka-IK-Abs-v0-100-v0/hdf_dataset.hdf5"
    # args.name = None 
    # args.debug = False
    # args.resume = False
    # main(args)
    ...
    # args = argparse.Namespace()
    # args.config = "robomimic/exps/templates/diffusion_policy.json"
    # args.dataset = "/home/hongchix/codes/simgen-mcp/server/logs/robomimic/Isaac-Lift-Single-Obj-Scene-Franka-IK-Abs-v0-aug-pose-v2-reach-80x2-train-set/hdf_dataset.hdf5"
    # args.name = "dp_mug_aug_pose_v2_reach_80x2"
    # args.debug = False
    # args.resume = True
    main(args)
